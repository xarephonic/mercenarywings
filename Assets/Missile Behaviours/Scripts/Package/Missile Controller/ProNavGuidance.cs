...
namespace MissileBehaviours.Controller
{
    ...
    public class ProNavGuidance : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("Wether or not to use the augmented version of the algorithm, which is a little more expensive to compute, but can improve the hitrate even further. Usually the normal mode is good enough, but in some cases the augmented version will show big improvements. For example in an air to air missile, launched at high velocity.")]
        bool advancedMode;
        ...
        void Update()
        {
            ...
            if (elapsedTime < delay)
                return;
            // Find out which target to use. If there is no active target, use the normal target instead.
            Vector3 activeTarget = controller.ActiveTarget ? controller.ActiveTarget.position : controller.Target.position;
            ...
            previousLos = los;
            los = activeTarget - transform.position;
