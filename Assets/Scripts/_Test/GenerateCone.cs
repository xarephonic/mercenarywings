...
public class GenerateCone : MonoBehaviour {
	...
	public void SetComplexity(int newcomp){
		complexity = newcomp;
	}
	...
	public void Cone(float h, float tR, float angle, int c){
		...
		float hypotenuse = Mathf.Cos(Mathf.Deg2Rad * angle/2) * h;

		float bottomRadius = topRadius + hypotenuse / Mathf.Sin(Mathf.Deg2Rad * angle/2);
	...
	void Start () {
		...
		angSlider.onValueChanged.AddListener(delegate(float arg0) {
			SetAngle(arg0);	
		});

		disSlider.onValueChanged.AddListener(delegate(float arg0) {
			SetDistance(arg0);	
		});
